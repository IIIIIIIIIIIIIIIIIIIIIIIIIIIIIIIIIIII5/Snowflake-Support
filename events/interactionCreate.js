import { ActionRowBuilder, ButtonBuilder, ButtonStyle, PermissionsBitField, EmbedBuilder, ChannelType } from "discord.js";
...
function GenerateTranscriptHtml(ChannelName, Messages) {
  ...
  `;
  let Html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><title>Transcript - ${ChannelName}</title><style>${Css}</style></head><body><h1>Transcript for #${ChannelName}</h1>`;
  Messages.reverse().forEach(Msg => {
    const Timestamp = new Date(Msg.createdTimestamp).toLocaleString();
    Html += `<div class="message"><img class="avatar" src="${Msg.author.displayAvatarURL({ format: 'png', size: 128 })}" alt="${Msg.author.tag}"><div class="content"><div class="header">${EscapeHtml(Msg.author.tag)} <span class="timestamp">${Timestamp}</span></div><div class="text">${EscapeHtml(Msg.content || '')}</div>`;
    ...
    if (Msg.stickers?.size) Msg.stickers.forEach(Sticker => { Html += `<img class="sticker" src="https://cdn.discordapp.com/stickers/${Sticker.id}.png" alt="${EscapeHtml(Sticker.name)}">`; });
    if (Msg.embeds?.length) Msg.embeds.forEach(Embed => {
      ...
      if (Embed.fields?.length) Embed.fields.forEach(F => { Html += `<div class="embed-field"><span class="embed-field-name">${EscapeHtml(F.name)}:</span><span class="embed-field-value">${EscapeHtml(F.value)}</span></div>`; });
...
async function UploadTranscript(ChannelId, Html) {
  ...
  try { await R2.send(Command); return `${process.env.R2PublicBase}/${Key}`; } 
  catch (Err) { console.error("R2 upload failed:", Err); return "https://example.com"; }
}
...
async function SyncPermissions(Channel, Category, OwnerId) {
  if (!Category) return;
  const Overwrites = Category.permissionOverwrites.cache.map(Po => ({ id: Po.id, allow: new PermissionsBitField(Po.allow).bitfield, deny: new PermissionsBitField(Po.deny).bitfield }));
...
export default {
  ...
  async execute(Interaction, Client) {
    ...
    for (const T of Object.values(ActiveTickets)) if (T.categoryType && typeof T.ticketNumber === "number") if (!Client.TicketCounts[T.categoryType] || Client.TicketCounts[T.categoryType] < T.ticketNumber) Client.TicketCounts[T.categoryType] = T.ticketNumber;
    ...
    if (Interaction.isStringSelectMenu() && Interaction.customId === "move_ticket") {
      ...
      try { const Owner = await Client.users.fetch(TicketData.ownerId); await Owner.send({ embeds: [new EmbedBuilder().setTitle("Ticket Moved").setColor("Yellow").setDescription(`Your ticket has been moved from **${OldCategory ? OldCategory.name : "Unknown"}** to **${NewCategory ? NewCategory.name : "Unknown"}**.`)] }); } catch (Err) { console.error("Failed to DM ticket owner:", Err); }
    ...
    if (Interaction.customId.startsWith("confirm_close_")) {
      ...
      let TranscriptUrl = "";
      try { TranscriptUrl = await UploadTranscript(Interaction.channel.id, Html); } catch (Err) { console.error("R2 upload failed:", Err); TranscriptUrl = "https://example.com"; }

      const CloseEmbed = new EmbedBuilder().setTitle("Ticket Closed").addFields({ name: "Ticket", value: Interaction.channel.name, inline: true }, { name: "Closed by", value: User.tag, inline: true }, { name: "Channel ID", value: Interaction.channel.id, inline: true }).setColor("Red").setTimestamp();
      ...
      const DmEmbed = new EmbedBuilder().setTitle("Ticket Closed").setColor("Red").addFields({ name: "Ticket", value: `${CategoryType} #${TicketNumber}`, inline: false }, { name: "Created At", value: CreatedAt.toLocaleString(), inline: true }, { name: "Closed At", value: `${ClosedAt.toLocaleString()} (${DiffDays} day${DiffDays !== 1 ? "s" : ""})`, inline: true }, { name: "Closed By", value: User.tag, inline: false });
      ...
      try { const Owner = await Client.users.fetch(TicketData.ownerId); await Owner.send({ embeds: [DmEmbed], components: [DmButton] }); } catch (Err) { console.error("Failed to DM user:", Err); }
    ...
    if (Interaction.customId === "close_ticket") {
      ...
      const ConfirmButtons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("confirm_close_yes").setLabel("Yes, close it").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId("confirm_close_no").setLabel("Cancel").setStyle(ButtonStyle.Secondary));
    ...
    if (Interaction.customId === "claim_ticket") {
      ...
      if (TicketMessage) { const UpdatedRow = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("close_ticket").setLabel("Close Ticket").setStyle(ButtonStyle.Danger)); await TicketMessage.edit({ components: [UpdatedRow] }); }
    ...
    switch (Interaction.customId) { case "report_ticket": CategoryId = TicketCategories.Report; Topic = "Report a User"; Type = "Report"; break; case "appeal_ticket": CategoryId = TicketCategories.Appeal; Topic = "Appeal a Punishment"; Type = "Appeal"; break; case "inquiry_ticket": CategoryId = TicketCategories.Inquiry; Topic = "Inquiries"; Type = "Inquiry"; break; default: return; }
    ...
    await SaveTickets(ActiveTickets);
    const Buttons = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("claim_ticket").setLabel("Claim Ticket").setStyle(ButtonStyle.Success), new ButtonBuilder().setCustomId("close_ticket").setLabel("Close Ticket").setStyle(ButtonStyle.Danger));
    const TicketEmbed = new EmbedBuilder().setTitle(`${Type} Ticket #${TicketNumber}`).setDescription(`Hello ${User}, your ticket has been created. A staff member will be with you shortly.`).setColor("Green").setTimestamp();
